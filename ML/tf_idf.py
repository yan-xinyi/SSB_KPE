...
def init_args():
    ...
    parser.add_argument('--TOPN', '-n', default=300, type=int,  help='The key phrases are ranking TopN.')
...
if __name__ == "__main__":
    ...
    save_path = os.path.join("./datas", args.dataset_name, 'tf_idf/results_%s1.json'%corpus_type) # 语料保存路径

    # Select different modes to run the program
    if args.mode == 'run':
        origin_datas = read_datas(file_path)
        words = data_preprocessing(origin_datas, ['title', 'abstract'], [2, 2])
        frequence = data_preprocessing(origin_datas, args.fields, [7] * len(args.fields))
        ref_of_merging_phrases = merge_datas_toline(words, 1)
        # ref_of_merging_phrases = merge_datas_toline(
        # data_preprocessing(origin_datas, args.fields, [2] * len(args.fields)), 1)
        datas = merge_datas_toline(frequence, 2)
